...
# Configure Upload & Snapshot Directories
UPLOAD_FOLDER = os.path.join(basedir, 'static', 'uploads', 'erp_photos')
SNAPSHOT_FOLDER = os.path.join(basedir, 'static', 'uploads', 'snapshots')
...
class Student(db.Model):
    ...
    # Phase 1 Facial Recognition Fields
    face_embedding = db.Column(db.Text, nullable=True) # JSON serialized 1404-D vector
...
class Attendance(db.Model):
    ...
    method = db.Column(db.String(50)) # 'QR', 'Barcode', or 'FACE'
    ...
    entry_method = db.Column(db.String(20)) # QR, MANUAL, BARCODE, FACE
    verification_status = db.Column(db.String(50)) # VERIFIED, FLAGGED
    # Phase 1 Face Attendance Fields
...
class SecurityAlert(db.Model):
    ...
    bus_no = db.Column(db.String(20))
    timestamp = db.Column(db.DateTime, default=datetime.datetime.utcnow)
    reason = db.Column(db.String(200)) # e.g. "Unrecognized Face", "Liveness Check Failed"
    snapshot_path = db.Column(db.String(300)) # Admin-restricted path
...
@app.route('/api/enroll-face/<int:student_id>', methods=['POST'])
@login_required
def enroll_face(student_id):
    ...
    # Save ERP photo for demo inspection (admin restricted)
    erp_photo_filename = f"student_{student.id}.jpg"
    erp_photo_path = os.path.join(UPLOAD_FOLDER, erp_photo_filename)
    cv2.imwrite(erp_photo_path, img)

    db.session.commit()
    print(f"[FACE ENROLL] Successfully enrolled face for {student.name} (ID: {student.id})")

    return jsonify({
        'status': 'success',
        'message': f'Face enrolled successfully for {student.name} (ID: {student.id}).',
        'face_enrolled': True
    })
...
def mark_attendance_face():
    ...
    for student in enrolled_students:
        if not student.face_embedding:
            continue
        try:
            stored_vec = json.loads(student.face_embedding)
            dist, sim = vision_helper.compute_vector_distance(current_embedding, stored_vec)
            print(f"[FACE MATCH DISTANCE] Target: {student.name} (ID: {student.id}) | Cosine Dist: {dist:.4f} | CosSim: {sim:.4f} | Limit: {MATCH_THRESHOLD}")
            if dist < min_dist:
                min_dist = dist
                best_student = student
        except Exception as e:
            print(f"[FACE ERROR] Parsing embedding for student {student.id}: {e}")
